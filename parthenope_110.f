...
C-----Differential equation resolution parameters
	...
	DIMENSION        CONST(6),RWORK(50+4*(NNUC+1)),RTOL(NNUC+1),
     .                 ATOL(NNUC+1),INFORM(23),YSAVE(NNUC+1,NY2DIM),
     .                 WKJAC(NWKJAC)
...
C-----Bessel functions routine
	INTEGER          IFAIL,NZ
	COMPLEX*16       ZR1,ZR2,ZR3,ZR4,ZR5,ZR6,ZR7
	COMPLEX*16       KZ1(4),KZ2(4),KZ3(4),KZ4(4),KZ5(4),KZ6(4),KZ7(4)
	EXTERNAL         S18DCF
C--------------------------Common variables-----------------------------
	DIMENSION        COEF(4)
	EQUIVALENCE      (ALP,COEF(1)),(BET,COEF(2)),(GAM,COEF(3)),
...
C-----Mass difference between neutron and proton in MeV
	q=mn-mp
	yy0(2)=1./(1.d0+ex(q/me*zin+xie))
	yy0(3)=1./(1.d0+ex(-q/me*zin-xie))
